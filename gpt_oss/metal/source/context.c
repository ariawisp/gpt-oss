    ...
    size_t num_output_tokens)
{
        ...
        input_batch_start += model->max_batch_tokens)
    {
        ...
        status = gptoss_metal_command_buffer_encode_launch_bf16_f32_embeddings(
            ...
            /*weight_offset=*/model->shared_weight_base_offset + 0,
        ...
        for (uint32_t n = 0; n < model->num_blocks; n++) {
            ...
            status = gptoss_metal_command_buffer_encode_launch_f32_bf16w_rmsnorm(
                ...
                /*input_offset=*/0,
                &model->shared_weight_buffer,
                /*weight_offset=*/model->shared_weight_base_offset + model->attn_rmsnorm_gain_offset + model->per_block_shared_weights_size * n,
            ...
            if (input_batch_size % dense_matmul_kernel_token_multiple_constraint == 0) {
                status = gptoss_metal_command_buffer_encode_launch_f32_bf16w_dense_matmul_qkv(
                    ...
                    /*input_offset=*/0,
                    &model->shared_weight_buffer,
                    /*weight_offset=*/model->shared_weight_base_offset + model->attn_qkv_weight_offset + model->per_block_shared_weights_size * n,
                    &model->shared_weight_buffer,
                    /*bias_offset=*/model->shared_weight_base_offset + model->attn_qkv_bias_offset + model->per_block_shared_weights_size * n,
            ...
            if (num_block_output_tokens != 0) {
                ...
                status = gptoss_metal_command_buffer_encode_launch_f32_bf16w_matmul_add(
                        ...
                        /*input_offset=*/0,
                        &model->shared_weight_buffer,
                        /*weight_offset=*/model->shared_weight_base_offset + model->attn_out_weight_offset + model->per_block_shared_weights_size * n,
                        &model->shared_weight_buffer,
                        /*bias_offset=*/model->shared_weight_base_offset + model->attn_out_bias_offset + model->per_block_shared_weights_size * n,
                        &context->residual_activation_buffer,
                        /*output_offset=*/model->embedding_dim * (input_batch_size - num_block_output_tokens) * sizeof(float),
                        &context->control_buffer,
                        /*control_offset=*/0,
                ...
                status = gptoss_metal_command_buffer_encode_launch_f32_bf16w_rmsnorm(
                    ...
                    &model->shared_weight_buffer,
                    /*weight_offset=*/model->shared_weight_base_offset + model->mlp_rmsnorm_gain_offset + model->per_block_shared_weights_size * n,
                    &context->rmsnorm_activation_buffer,
                    /*output_offset=*/0,
                    &context->control_buffer,
                    /*control_offset=*/0,
                ...
                if (input_batch_size % dense_matmul_kernel_token_multiple_constraint == 0) {
                    status = gptoss_metal_command_buffer_encode_launch_f32_bf16w_dense_matmul_mlp_gate(
                        ...
                        /*input_offset=*/0,
                    &model->shared_weight_buffer,
                    /*weight_offset=*/model->shared_weight_base_offset + model->mlp_gate_weight_offset + model->per_block_shared_weights_size * n,
                    &model->shared_weight_buffer,
                    /*bias_offset=*/model->shared_weight_base_offset + model->mlp_gate_bias_offset + model->per_block_shared_weights_size * n,
